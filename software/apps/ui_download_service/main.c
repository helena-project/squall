...
//definitions related to UI download service
#define UI_SIZE 3668
#define COMPRESSION_FORMAT 0x01
//#define NUM_CHARS CEILING(UI_SIZE, 512) //characterisitc size is 512
#define UI_START			0x00018800
...
static void on_conn_params_evt(ble_conn_params_evt_t * p_evt)
{
	...
	{
		err_code = sd_ble_gap_disconnect(ui.conn_handle, BLE_HCI_CONN_INTERVAL_UNACCEPTABLE);
		APP_ERROR_CHECK(err_code);
	}
}
...
static void on_ble_evt(ble_evt_t * p_ble_evt) {
	...
	switch (p_ble_evt->header.evt_id) {
		case BLE_GAP_EVT_CONNECTED:
			ui.conn_handle = p_ble_evt->evt.gap_evt.conn_handle;
			//advertisingStop();
			break;

		case BLE_GAP_EVT_DISCONNECTED:
			ui.conn_handle = BLE_CONN_HANDLE_INVALID;
			advertisingStart();
			break;

		case BLE_GAP_EVT_SEC_PARAMS_REQUEST:
			err_code = sd_ble_gap_sec_params_reply(ui.conn_handle,
		...
		case BLE_GATTS_EVT_SYS_ATTR_MISSING:
			err_code = sd_ble_gatts_sys_attr_set(ui.conn_handle, NULL, 0);
			APP_ERROR_CHECK(err_code);
			break;
		...
		case BLE_GAP_EVT_SEC_INFO_REQUEST:
			...
			if (p_enc_info->div ==
				...
				err_code = sd_ble_gap_sec_info_reply(ui.conn_handle,
															p_enc_info, NULL);
				APP_ERROR_CHECK(err_code);
			} else {
				// No keys found for this device
				err_code = sd_ble_gap_sec_info_reply(ui.conn_handle, NULL, NULL);
				APP_ERROR_CHECK(err_code);
			}
			break;
...
static void services_init(void)
{
	...
	// as a handle.
	custom_service_uuid.uuid = UI_SHORT_UUID;
	err_code = sd_ble_uuid_vs_add(&custom_service_uuid128, &(custom_service_uuid.type));
	APP_ERROR_CHECK(err_code);
	ui.uuid_type = custom_service_uuid.type;
	...
	//add the comp characterisitc
	{
		...
		char_uuid.type = ui.uuid_type;
		char_uuid.uuid = UI_SERVICE_COMPTYPE_SHORT_UUID;
		...
		attr_char_value.p_value   = (uint8_t*) &ui.comp_value;

		err_code = sd_ble_gatts_characteristic_add(ui.service_handle,
		                                           &char_md,
		                                           &attr_char_value,
		                                           &ui.comp_handles);
		APP_ERROR_CHECK(err_code);
	...
	//add the comp characterisitc
	{
		...
		char_uuid.type = ui.uuid_type;
		char_uuid.uuid = UI_SERVICE_LEN_SHORT_UUID;
		...
		attr_char_value.p_value   = (uint8_t*) &ui.len_value;
	...
	for(uint8_t char_num = 0; char_num < ceil(UI_SIZE/512.0); char_num++)
	{
		ble_gatts_char_md_t char_md;
		ble_gatts_attr_t    attr_char_value;
		ble_uuid_t          char_uuid;
		...
		// Init value
		ui.ui_pointer = UI_START + (512*char_num);
		...
		char_uuid.type = ui.uuid_type;
		char_uuid.uuid = UI_SERVICE_UI_BASE_SHORT_UUID + char_num;
		...
		attr_char_value.p_uuid    = &char_uuid;
		attr_char_value.p_attr_md = &attr_md;
		if(UI_SIZE-(512*char_num) >= 512) {
			attr_char_value.init_len  = 512;
			attr_char_value.init_offs = 0;
			attr_char_value.max_len   = 512;
		} else {
			attr_char_value.init_len  = UI_SIZE-(512*char_num); 
			attr_char_value.init_offs = 0;
			attr_char_value.max_len   = UI_SIZE-(512*char_num);
		}

		attr_char_value.p_value   = ui.ui_pointer;
