...
static void on_write(ble_nus_t * p_nus, ble_evt_t * p_ble_evt)
{
       ...
       )
    {
        ...
        ble_nus_send_string(p_nus, "You connected!", strlen("You connected!"));
    }
    else if (
            ...
            )
    {
        ...
        ble_nus_set_read_string(p_nus, p_evt_write->data, p_evt_write->len);
    }
    else
    {
...
static uint32_t read_char_add(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init)
{
    ...
    ble_uuid.uuid             = BLE_UUID_NUS_READ_CHARACTERISTIC;
    ...
    attr_char_value.max_len   = BLE_NUS_MAX_READ_CHAR_LEN;
    attr_char_value.p_value     = read_data;
...
uint32_t ble_nus_init(ble_nus_t * p_nus, const ble_nus_init_t * p_nus_init)
{
    ...
    // data buffer can be updated whenever
    memset(read_data, 'a', 500);

    return NRF_SUCCESS;
}

//NOTE: This length is limited to 20 bytes
...
{
    return sd_ble_gatts_value_set(p_nus->read_handles.value_handle, 0, &length, string);
}
